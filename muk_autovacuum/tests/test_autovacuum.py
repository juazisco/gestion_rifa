...
class AutoVacuumTestCase(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.model_logs = self.model_model.search(
            [('model', '=', 'ir.logging')], limit=1)
        time_field_domain = [
            ('model_id', '=', self.model_logs.id),
            ('ttype', '=', 'datetime'),
            ('name', '=', 'create_date')]
        self.time_field_logs = self.model_fields.search(
            time_field_domain, limit=1)
    ...
    def run_autovacuum(self):
        ...
        self.env['ir.cron'].search(
            [('model_id', '=', 'ir.autovacuum')]).ir_actions_server_id.run()
