...
class Base(models.AbstractModel):
    ...
    def _check_parent_field(self):
        if self._parent_name not in self._fields:
            raise TypeError(
                "The parent (%s) field does not exist." %
                self._parent_name)
    ...
    def _build_search_childs_domain(self, parent_id, domain=[]):
        ...
        return expression.AND([parent_domain, domain]
                              ) if domain else parent_domain

    @api.model
    def _check_context_bin_size(self, field):
        return any(key in self.env.context for key in [
                   'bin_size', 'bin_size_%s' % (field)])
    ...
    @api.model
    def search_parents(self, domain=[], offset=0,
    ...
    @api.model
    def search_read_parents(self, domain=[], fields=None,
    ...
    @api.model
    def _search_parents(self, domain=[], offset=0,
                        limit=None, order=None, count=False):
        self._check_parent_field()
    ...
    @api.model
    def search_childs(self, parent_id, domain=[], offset=0,
                      limit=None, order=None, count=False):
        ...
        return self.search(domain, offset=offset,
                           limit=limit, order=order, count=count)

    @api.model
    def search_read_childs(self, parent_id, domain=[],
