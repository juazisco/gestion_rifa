...
class Hierarchy(models.AbstractModel):
    ...
    def _search_parent_path_names(self, operator, operand):
        ...
        return domain if domain else [
            (self._rec_name_fallback(), operator, "")]
    ...
    def _compute_parent_paths(self):
        ...
        paths = [list(map(int, rec.parent_path.split('/')[:-1]))
                 for rec in records]
        ids = paths and set(functools.reduce(operator.concat, paths)) or []
        model_without_path = self.with_context(
            **{self._name_path_context: False})
        filtered_records = model_without_path.browse(
            ids)._filter_access('read')
