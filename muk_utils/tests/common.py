...
def multi_users(users=[['base.user_root', True], [
                'base.user_admin', True]], reset=True, raise_exception=True):
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if test_fails:
                message = "%s out of %s tests failed" % (
...
def track_function(max_query_count=None, max_query_time=None,
                   max_time=None, return_tracking=False):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            ...
            if hasattr(threading.current_thread(), "query_count"):
                ...
                if max_query_time and query_time > max_query_time:
                    raise AssertionError(
                        "Queries took longer than %.3fs" %
                        max_query_time)
                if max_time and time_taken > max_time:
                    raise AssertionError(
                        "Function took longer than %.3fs" %
                        max_time)
